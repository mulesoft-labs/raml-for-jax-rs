...
public class CurrentBuild {
    ...
    private final ArrayListMultimap<JavaPoetTypeGenerator, JavaPoetTypeGenerator> internalTypesPerClass = ArrayListMultimap.create();
    ...
    public CurrentBuild(Document api, ExtensionManager extensionManager) {
        ...
        this.ramlToPojo = Suppliers.memoize(() ->
                RamlToPojoBuilder.builder(this.api)
                        .inPackage(getModelPackage())
                        .fetchTypes(TypeFetchers.fromAnywhere())
                        .build(this.typeConfiguration().stream().map((Function<String, String>) s -> s.contains(".") ? s : "core." + s).collect(Collectors.toList())));
    }
    ...
    public void generate(final File rootDirectory) throws IOException {
        ...
        try {
            ...
            for (JavaPoetTypeGenerator typeGenerator : supportGenerators) {
                ...
                typeGenerator.output(g -> JavaFile.builder(getSupportPackage(), g.build()).skipJavaLangImports(true).build().writeTo(rootDirectory));
            }
    ...
    public ResourceClassExtension pluginsForResourceClass(Function<Collection<ResourceClassExtension>, ResourceClassExtension> provider,
        ...
        List<PluginDef> data = Collections.emptyList(); // todo annotations.... Annotations.RESOURCE_PLUGINS.get(Collections.<PluginDef>emptyList(), api, resource);
    ...
    public ResponseClassExtension pluginsForResponseClass(Function<Collection<ResponseClassExtension>, ResponseClassExtension> provider,
        ...
        List<PluginDef> data = Collections.emptyList(); // todo Annotations.RESPONSE_CLASS_PLUGINS.get(Collections.<PluginDef>emptyList(), api, method);
    ...
    public ResourceMethodExtension pluginsForResourceMethod(Function<Collection<ResourceMethodExtension>, ResourceMethodExtension> provider,
        ...
        List<PluginDef> data = Collections.emptyList(); // todo Annotations.METHOD_PLUGINS.get(Collections.<PluginDef>emptyList(), api, resource);
    ...
    public ResponseMethodExtension pluginsForResponseMethod(Function<Collection<ResponseMethodExtension>, ResponseMethodExtension> provider,
        ...
        List<PluginDef> data = Collections.emptyList(); // todo Annotations.RESPONSE_PLUGINS.get(Collections.<PluginDef>emptyList(), api, resource);
