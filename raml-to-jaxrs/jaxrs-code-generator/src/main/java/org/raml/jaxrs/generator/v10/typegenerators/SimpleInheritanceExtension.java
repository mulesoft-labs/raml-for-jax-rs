...
public class SimpleInheritanceExtension implements TypeExtension {
    ...
    private TypeSpec.Builder buildTypeImplementation(TypeContext context, V10GType objectType) {
        ...
        for (GProperty declaration : originalType.properties()) {
            ...
            if (declaration.isInline()) {
                ...
                V10GType type = localRegistry.createInlineType(internalTypeName,
                        Annotations.CLASS_NAME.get(Names.typeName(declaration.name(), "Type"), (Annotable) declaration.implementation()),
    ...
    private void buildPropertiesForImplementation(TypeContext context, V10GType objectType, TypeSpec.Builder typeSpec, List<PropertyInfo> properties) {
        for (PropertyInfo propertyInfo : properties) {
            ...
            fieldSpec = context.onField(context, typeSpec, fieldSpec, objectType, (V10GProperty) propertyInfo.getProperty(), BuildPhase.IMPLEMENTATION,
                    PredefinedFieldType.PROPERTY);

            fieldSpec = currentBuild.getFieldExtension(Annotations.ON_TYPE_FIELD_CREATION, objectType).onField(context, typeSpec, fieldSpec, objectType,
                    (V10GProperty) propertyInfo.getProperty(), BuildPhase.IMPLEMENTATION, PredefinedFieldType.PROPERTY);
            ...
            MethodSpec.Builder getSpec = MethodSpec.methodBuilder(Names.methodName("get", Names.typeName(propertyInfo.getName()))).addModifiers(Modifier.PUBLIC)
                    .addStatement("return this." + Names.variableName(propertyInfo.getName()));

            getSpec.returns(propertyInfo.resolve(context));
            getSpec = runMethodExtensions(context, objectType, typeSpec, getSpec, Collections.<ParameterSpec.Builder> emptyList(),
                    (V10GProperty) propertyInfo.getProperty(), BuildPhase.IMPLEMENTATION, PredefinedMethodType.GETTER, Annotations.ON_TYPE_METHOD_CREATION);
            ...
            if (!propertyInfo.getName().equals(objectType.discriminator())) {
                ...
                MethodSpec.Builder setSpec = MethodSpec.methodBuilder(Names.methodName("set", Names.typeName(propertyInfo.getName())))
                        .addModifiers(Modifier.PUBLIC)
                        .addStatement("this." + Names.variableName(propertyInfo.getName()) + " = " + Names.variableName(propertyInfo.getName()));

                ParameterSpec.Builder parameterSpec = ParameterSpec.builder(propertyInfo.resolve(context), Names.variableName(propertyInfo.getName()));

                setSpec.addParameter(parameterSpec.build());

                setSpec = runMethodExtensions(context, objectType, typeSpec, setSpec, Collections.singletonList(parameterSpec),
                        (V10GProperty) propertyInfo.getProperty(), BuildPhase.IMPLEMENTATION, PredefinedMethodType.SETTER, Annotations.ON_TYPE_METHOD_CREATION);
    ...
    private TypeSpec.Builder buildDeclaration(TypeContext context, V10GType objectType) {
        ...
        TypeSpec.Builder typeSpec = TypeSpec.interfaceBuilder(interf)
                // .addAnnotation(AnnotationSpec.builder(RamlGenerator.class).addMember("value",
                // "$S", "ramlforjaxrs-simple").build())
                .addModifiers(Modifier.PUBLIC);
    ...
    private void buildPropertiesForInterface(TypeContext context, V10GType objectType, List<PropertyInfo> properties, TypeSpec.Builder typeSpec) {
        for (PropertyInfo propertyInfo : properties) {
            ...
            MethodSpec.Builder getSpec = MethodSpec.methodBuilder(Names.methodName("get", propertyInfo.getName())).addModifiers(Modifier.PUBLIC,
                    Modifier.ABSTRACT);
            getSpec.returns(propertyInfo.resolve(context));

            getSpec = runMethodExtensions(context, objectType, typeSpec, getSpec, Collections.<ParameterSpec.Builder> emptyList(),
                    (V10GProperty) propertyInfo.getProperty(), BuildPhase.INTERFACE, PredefinedMethodType.GETTER, Annotations.ON_TYPE_METHOD_CREATION);
            ...
            if (!propertyInfo.getName().equals(objectType.discriminator())) {
                MethodSpec.Builder setSpec = MethodSpec.methodBuilder(Names.methodName("set", propertyInfo.getName())).addModifiers(Modifier.PUBLIC,
                        Modifier.ABSTRACT);
                ParameterSpec.Builder parameterSpec = ParameterSpec.builder(propertyInfo.resolve(context), Names.variableName(propertyInfo.getName()));
                ...
                setSpec = runMethodExtensions(context, objectType, typeSpec, setSpec, Collections.singletonList(parameterSpec),
                        (V10GProperty) propertyInfo.getProperty(), BuildPhase.INTERFACE, PredefinedMethodType.SETTER, Annotations.ON_TYPE_METHOD_CREATION);
    ...
    private MethodSpec.Builder runMethodExtensions(TypeContext context, V10GType objectType, TypeSpec.Builder typeSpec, MethodSpec.Builder getSpec,
            List<ParameterSpec.Builder> parameters, V10GProperty property, BuildPhase phase, MethodType methodType, Annotations<MethodExtension> annotations) {

        MethodSpec.Builder builder = context.onMethod(context, typeSpec, getSpec, parameters, objectType, property, phase, methodType);

        builder = currentBuild.getMethodExtension(annotations, objectType).onMethod(context, typeSpec, builder, parameters, objectType, property, phase,
                methodType);
        return builder;
    }

    private TypeSpec.Builder runClassExtensions(TypeContext context, V10GType objectType, TypeSpec.Builder typeSpec, BuildPhase buildPhase,
