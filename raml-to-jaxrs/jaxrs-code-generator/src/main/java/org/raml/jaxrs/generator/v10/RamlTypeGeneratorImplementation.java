 ...
 */
public class RamlTypeGeneratorImplementation extends AbstractTypeGenerator<TypeSpec.Builder>
  ...
  public RamlTypeGeneratorImplementation(CurrentBuild build, ClassName className,
                                         ClassName parentClassName, List<PropertyInfo> properties,
  ...
  public void output(CodeContainer<TypeSpec.Builder> container, TYPE type) throws IOException {
    ...
    build.withTypeListeners().onTypeImplementation(build, typeSpec,
    ...
    for (PropertyInfo propertyInfo : propertyInfos.values()) {
      ...
      build.withTypeListeners().onFieldImplementation(build, fieldSpec,
      ...
      final MethodSpec.Builder getSpec =
          MethodSpec.methodBuilder("get" + Names.typeName(propertyInfo.getName()))
              .addModifiers(Modifier.PUBLIC)
              .addStatement("return this." + Names.variableName(propertyInfo.getName()));

      getSpec.returns(propertyInfo.resolve(build));
      build.withTypeListeners().onGetterMethodImplementation(build, getSpec,
                                                             (TypeDeclaration) propertyInfo.getType().implementation());
      ...
      if (!propertyInfo.getName().equals(object.discriminator())) {
        ...
        MethodSpec.Builder setSpec =
            MethodSpec
                ...
                .addModifiers(Modifier.PUBLIC)
                .addStatement(
                              "this." + Names.variableName(propertyInfo.getName()) + " = "
        ...
        ParameterSpec.Builder parameterSpec =
            ParameterSpec.builder(propertyInfo.resolve(build),
                                  Names.variableName(propertyInfo.getName()));
        build.withTypeListeners().onSetterMethodImplementation(build, setSpec, parameterSpec,
                                                               (TypeDeclaration) propertyInfo.getType().implementation());
